...
/** Compare the mission to the saved baseline (if any); does NOT write. */
export async function diffBaseline(dir: string, key: string, mission: MissionGraph): Promise<BaselineDiff> {
  const now = fingerprints(mission)
  const nowSet = new Set(now)
  const file = await contained(dir, key)
  let prior: { savedAt?: number; fingerprints?: string[] } | null = null
  try {
    prior = JSON.parse(await fs.readFile(file, 'utf8'))
  } catch {
    prior = null
  }
  if (!prior || !Array.isArray(prior.fingerprints)) return { first: true, added: now, resolved: [], persisting: [], since: null }
...
/** Persist the current fingerprints as the new baseline. Never throws. */
export async function saveBaseline(dir: string, key: string, mission: MissionGraph, now: number): Promise<void> {
  try {
    const file = await contained(dir, key)
    await fs.writeFile(file, JSON.stringify({ key, savedAt: now, fingerprints: fingerprints(mission) }), 'utf8')
  } catch {
...
export function driftClaim(diff: BaselineDiff, missionId: string, now: number, goalId?: string): CognitiveClaim {
  ...
  return newClaim(
    {
      missionId, goalId, sense: 'memory', operation: 'learn', capability: 'drift',
      verdict,
      confidence: 0.8,
      evidence: diff.added.slice(0, 12).map((f) => ({ what: `[regression] appeared since last audit: ${f}`, framed: true })),
      unknowns: diff.resolved.slice(0, 12).map((f) => `resolved since last audit (verify it stays fixed): ${f}`),
